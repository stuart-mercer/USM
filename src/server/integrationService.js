...
module.exports = class IntegrationService {
    ...
    /**
     * Gets Opportunity records from Salesforce
     * @param {Object} req - server request
     * @param {Object} res - server response
     */
    getOppDetails(req, res) {
        const session = this.authService.getSession(req, res);
        if (session === null) {
            return;
        }
        ...
        let soqlQuery;
        if (req.params.id) {
            // Retrieve details of a specific Opportunity with a given Id
            soqlQuery = `SELECT Id, Name FROM Opportunity WHERE Id = '${req.params.id}'`;
        } else {
        ...
        this._runSoql(conn, soqlQuery)
            .then((records) => {
                // Format data
                const formattedData = records.map((oppRecord) => {
                    return {
                        id: oppRecord.Id,
                        name: oppRecord.Name
                    };
